...
<div class="flex flex-col sm:flex-row " style="direction: ltr;">
    <div class=" w-full sm:w-2/3 md:w-3/4 text-black font-light text-lg text-left px-4 py-2 m-2 leading-relaxed">
        <time datetime="{{ $page->date }}" class="font-serif font-thin text-gray-800">{{ date('d M, Y', $page->date) }}</time>
        <h1 class="font-serif text-2xl sm:text-3xl md:text-5xl font-medium mb-3 mt-0">{{ $page->title }}</h1>
        @if ($page->cover_image)
